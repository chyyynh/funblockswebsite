import Image from "next/image";
import Link from "next/link";

interface GameCardProps {
  id: number;
  title: string;
  slug: string;
  image: string;
  blockchain: string;
  engine: string;
  type: string;
  status: string;
  twitter_img: { pfp: string; banner: string };
  link: { website?: string; twitter?: string };
}

export function GameList(game: GameCardProps) {
  return (
    <tr key={game.id} className="hover:bg-gray-50 transition-colors">
      <td className="py-1 px-4 border-b text-gray-500">index</td>
      <td className="py-1 px-4 border-b">
        <div className="flex items-center gap-3">
          {/*
      <div className="relative h-10 w-20 overflow-hidden rounded-md">
        <Image
          src={game.twitter_img.banner || "/placeholder.svg"}
          alt={game.title}
          fill
          className="object-cover"
        />
      </div>
      */}
          <div className="relative h-10 w-10 overflow-hidden rounded-md">
            <Image
              src={game.twitter_img.pfp || "/placeholder.svg"}
              alt={game.title}
              fill
              className="object-cover"
            />
          </div>
          <div>
            <Link
              href={`/games/${game.slug}`}
              className="font-semibold hover:underline"
            >
              {game.title}
            </Link>
            <div className="flex items-center gap-1 mt-1">
              {game.engine && (
                <span className="text-xs text-gray-500 bg-gray-100 px-1.5 py-0.5 rounded">
                  {game.engine}
                </span>
              )}
              {game.blockchain && (
                <span className="text-xs text-gray-500 bg-gray-100 px-1.5 py-0.5 rounded">
                  {game.blockchain}
                </span>
              )}
            </div>
          </div>
        </div>
      </td>
      <td className="py-1 px-4 border-b text-center text-gray-600">
        players: no data
      </td>
      <td className="py-1 px-4 border-b text-center text-gray-600">
        followers: no data
      </td>
      <td className="py-1 px-4 border-b text-center">
        <span className="inline-block px-2 py-1 bg-gray-100 rounded-full text-xs">
          {game.status || "Unknown"}
        </span>
      </td>
      <td className="py-1 px-4 border-b text-right">
        <div className="flex items-center justify-end gap-1">
          {Array.from({
            length: Math.floor(3),
          }).map((_, i) => (
            <span key={i} className="h-2 w-2 bg-[#F3B43B] rounded-full"></span>
          ))}
          {Array.from({
            length: 5 - Math.floor(3),
          }).map((_, i) => (
            <span key={i} className="h-2 w-2 bg-gray-200 rounded-full"></span>
          ))}
        </div>
      </td>
    </tr>
  );
}
